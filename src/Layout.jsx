    ...
    <div className="flex items-start h-full">
      <div className="bg-custom-primary sticky top-0 p-10 flex flex-col justify-between text-white h-screen">
        <div className="flex flex-col gap-6">
          <img loading="lazy" alt="Logo de IntheairLabs" className="w-[200px]" src="/src/assets/intheairlabs_blanc.png" />
                ...
                <div className="flex flex-col flex-no-wrap">
                  {section.links.map((link, linkIndex) => (
                    <a
                      key={linkIndex}
                      href={link.href}
                      className="flex items-center gap-2 text-sm whitespace-nowrap hover:bg-white/10 py-2 px-2 rounded-sm duration-150"
                    >
      ...
      <div className="p-12 w-full flex flex-col gap-10">{children}</div>
    </div>
  );
}
